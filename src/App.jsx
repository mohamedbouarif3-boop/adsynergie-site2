...
function ServicesCarousel({ inView }) {
...
  
  // Utilisez la prop 'inView' passée par App
  return (
    <section id="services" className={`py-12 sm:py-16 transition-all duration-1000 delay-200 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
...
              <div
                key={i}
                className="min-w-full p-6 sm:p-8 hover:bg-slate-50 transition duration-300" 
                role="group"
...
function PricingCarousel({ inView }) {
  const [index, setIndex] = useState(0);
...
  // Utilisez la prop 'inView' passée par App
  return (
    <section id="pricing" className={`py-12 sm:py-16 transition-all duration-1000 delay-200 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
...
function TestimonialsSlider({ inView }) {
  const [index, setIndex] = useState(0);
...
  return (
    <section id="testimonials" className={`py-16 bg-brand-teal/5 transition-all duration-1000 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
...
  // 🚨 CORRECTION MAJEURE: Centralisation des appels useInView
  const [servicesRef, servicesInView] = useInView({ threshold: 0.1, once: true });
  const [methodRef, methodInView] = useInView({ threshold: 0.1, once: true });
  const [pricingRef, pricingInView] = useInView({ threshold: 0.1, once: true });
...
        {/* SERVICES (carrousel) */}
        <div ref={servicesRef}>
          <ServicesCarousel inView={servicesInView} />
        </div>

        {/* MÉTHODE (Animation) */}
        <section id="process" ref={methodRef} className="py-12 sm:py-16">
...
            {["1. Analyse & stratégie", "2. Création & personnalisation", "3. Leads exclusifs & suivi"].map((title, i) => {
                // Utilisation de l'état methodInView global pour animer les blocs
                return (
                    <div key={i} className={`p-6 border rounded-2xl bg-white/80 transition-all duration-700 hover:shadow-lg ${methodInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{ transitionDelay: `${i * 100}ms` }}>
...
        {/* TARIFS (carrousel) */}
        <div ref={pricingRef}>
          <PricingCarousel inView={pricingInView} />
        </div>
